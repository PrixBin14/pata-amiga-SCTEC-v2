...
-- =====================================================================================
--  Rodar depois do: 01-carga-staging.sql
--
--  Aqui ja vem pronto:
--
--  1) dim_tempo e dim_loja, ja carregadas e prontas pra usar. Nao precisei
--     construir essas duas - so aproveitar.
--
--  2) O CREATE TABLE das outras quatro tabelas (dim_categoria, dim_praca,
--     bridge_loja_praca e fato_pedido), todas vazias por enquanto. Elas vao
--     ser preenchidas nos arquivos 03 e 04.
--
--  dim_tempo   A chave e a propria data virada numero: 16/11/2023 vira
--              20231116. Assim a fato monta essa FK so com calculo, sem
--              precisar de JOIN.
--
--  dim_loja    Uma linha por loja, mais a linha -1. A coluna chave_loja tem o
--              nome ja padronizado (caixa alta, sem acento) - e por ela que a
--              fato vai encontrar a loja certa depois de limpar o nome que
--              vem da stg_pedido.
--
--  Toda dimensao tem a linha -1 pra "Nao Informado". Nenhuma FK fica nula:
--  quando falta o dado, ela aponta pra essa linha.
-- =====================================================================================
...
-- =====================================================================================
--  DIM_LOJA  (ja vem pronta)
-- =====================================================================================
--  32 lojas mais a linha -1. O porte vem da area de venda: Pequena < 150 m2,
--  Media entre 150 e 400, Grande > 400. E por esse campo que a P1 e a P3
--  agrupam. E a populacao_cidade e o denominador que uso la na P5, pra tirar
...
-- =====================================================================================
--  AS TABELAS QUE EU VOU PREENCHER MAIS PRA FRENTE  -  ja criadas, vazias
-- =====================================================================================
--  Abaixo estao os CREATE TABLE das quatro tabelas que ainda faltam. Elas
--  ficam vazias por enquanto; eu preencho nos arquivos 03 e 04.
--
--  Os comentarios do lado de cada coluna sao mais ou menos um lembrete do que
--  vai entrar ali.
...
-- -------------------------------------------------------------------------------------
--  Essa e a tabela PONTE. Como uma loja entrega em mais de uma praca e uma FK
--  so guarda um valor, a ligacao N:N precisa morar numa tabela separada, com
--  o fator de rateio dentro dela.
--
--  Detalhe pra nao esquecer: a ponte se liga pelo COD DA LOJA, nao pela sk_loja.
...
CREATE TABLE fato_pedido (
    ...
    -- as duas FKs de tempo: a mesma dim_tempo em dois papeis diferentes
    sk_tempo_pedido  INT NOT NULL,
    sk_tempo_entrega INT NOT NULL,  -- fica -1 se a entrega ainda nao aconteceu

    -- as outras FKs. Nenhuma fica nula: quando falta o dado, vale -1
    sk_loja          INT NOT NULL,
    sk_categoria     INT NOT NULL,

    -- atributos que moram direto na fato
    -- desconto e canal sao dois campos com poucos valores possiveis. Como nao
    -- tem nada mais "pendurado" neles pra virar dimensao, deixei aqui mesmo na
    -- fato, ja padronizados no arquivo 04.
    ...
    -- os lags do processo todo, em dias. Calculo eles uma unica vez, aqui na
    -- carga. Etapa que ainda nao aconteceu grava NULL, nunca 0.
